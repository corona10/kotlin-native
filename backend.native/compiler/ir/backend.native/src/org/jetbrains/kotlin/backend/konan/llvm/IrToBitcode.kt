...
internal fun emitLLVM(context: Context) {
    ...
    phaser.phase(KonanPhase.CODEGEN) {
        val visitor = CodeGeneratorVisitor(context, lifetimes)
        visitor.codegen.staticData.placeGlobalConstArray("private_functions_${irModule.descriptor.name}", int8TypePtr,
                devirtualizationAnalysisResult!!.privateFunctions.map { constPointer(visitor.codegen.functionLlvmValue(it)).bitcast(int8TypePtr) },
                isExported = true
        )
